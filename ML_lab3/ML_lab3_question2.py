...
with open("data/movie-pang02.csv") as f:
    data = []
...
num_total_train_data = int(num_total_data * 0.9)
num_total_test_data = int(num_total_data * 0.1)
...
while i < num_total_train_data:
    ...
    if not training_data_indices[random_index] == 1:
        ...
        training_data.append(data[random_index].split())
        i = i + 1
...
num_positives_used_to_train = 0.0
num_negatives_used_to_train = 0.0
num_positives_used_to_test = 0.0
num_negatives_used_to_test = 0.0

for k in range(1, len(training_data)):
    if not found_split:
        index_of_split = k
    if not training_data[k][0] == "Neg":
...
num_positives_used_to_test = int((num_total_train_data - num_positives_used_to_train)/2)
num_negatives_used_to_test = int((num_total_train_data - num_negatives_used_to_train)/2)


words_list = {}

for i in range(index_of_split, len(training_data)):  #Add the positive review's words into the word list
    review = training_data[i][1:]
    for word in review:
...
for i in range(0, index_of_split):   #Add the negative review's words into the word list
    review = training_data[i][1:]      #Begins where above loop left of: index_of_split
    for word in review:
        if word in words_list:
            if not words_list[word][5] == 1:
                words_list[word][0] = words_list[word][0] + 1
...
for word in words_list:
    if abs(words_list[word][3] - words_list[word][4]) < 0.06:
...
def prob_input_given_positive_logs(split_review):
    likelihood = 1.0
    input_vector = convert_review_to_input_vector(split_review)
    for index in range(0, len(input_vector)):
        if input_vector[index] == 1:
            likelihood = likelihood + (math.log(words_list[words_vector[index]][3],2))
        else:
            likelihood = likelihood + (math.log(1 - words_list[words_vector[index]][3], 2))
    return likelihood
...
def prob_input_given_negative_logs(split_review):
    likelihood = 1.0
    input_vector = convert_review_to_input_vector(split_review)
    for index in range(0, len(input_vector)):
        if input_vector[index] == 1:
            likelihood = likelihood + math.log((words_list[words_vector[index]][4]),2)
        else:
            likelihood = likelihood + (math.log(1 - words_list[words_vector[index]][4]))
    return likelihood

def prob_input_across_all_condition(split_review):
    probability = (prob_input_given_positive_logs(split_review) * training_prior_prob_positive) + (prob_input_given_negative_logs(split_review) * training_prior_prob_negative)
    return probability

def classify(split_review):
    positive_posterior_probability = ((prob_input_given_positive_logs(split_review) * training_prior_prob_positive))/(prob_input_across_all_condition(split_review))
    negative_posterior_probability = ((prob_input_given_negative_logs(split_review) * training_prior_prob_negative))/(prob_input_across_all_condition(split_review))
    print "Pos Posterior= " + str(positive_posterior_probability) + " Neg posterior= " + str(negative_posterior_probability)
...
def main():
    ...
    for review in test_data:
        categorization = compare_result(review)
        if review[0] == "Pos" and  categorization == 1:
            correct_pos = correct_pos + 1
        elif review[0] == "Neg" and  categorization == 1:
            correct_neg = correct_neg + 1
        elif review[0] == "Pos" and  categorization == 0:
            miss_pos = miss_pos + 1
        else:
            miss_neg = miss_neg + 1
